...
class PackingController extends BaseController
{
    ...
    public function datatable_packing(Request $request)
    {
        ...
        $data = $this->packing->dataTablePacking($start_date, $end_date, $status, $order_by); 
                ...
                ->addColumn('action', function($row) {
                    ...
                    if(Gate::allows('crudAccess', 'PACK', $row)) {
                        ...
                        if(in_array($user->id_role, [1,2])) {
                            $btn_action .= '<li><a href="/packing/form/'.$row->uid.'" class="btn"><em class="icon ni ni-edit"></em><span>Edit</span></a></li>
    ...
    public function form_packing(Request $request)
    {
        $title      = 'Form Job';
        $id         = $request->id;
        $js         = 'js/apps/packing/form.js?_='.rand();
        $divisi     = DB::table('divisi')->where([['status', 1], ['urutan', '<>', 0]])->orderBy('urutan', 'asc')->get();

        return view('packing.form', compact('title', 'js', 'id', 'divisi'));
    }
    ...
    public function store_packing(Request $request)
    {
        $id = $request->input('id');
