...
class TambahanController extends BaseController
{
    ...
    public function datatable_tambahan(Request $request)
    {
        ...
        $data = $this->tambahan->dataTableTambahan($start_date, $end_date, $status, $order_by); 
                ...
                ->addColumn('action', function($row) {
                    ...
                    if(Gate::allows('crudAccess', 'PACK1', $row)) {
                        ...
                        if(in_array($user->id_role, [1,2])) {
                            $btn_action .= '<li><a href="/tambahan/form/'.$row->uid.'" class="btn"><em class="icon ni ni-edit"></em><span>Edit</span></a></li>
    ...
    public function form_tambahan(Request $request)
    {
        $title      = 'Form Job';
        $id         = $request->id;
        $js         = 'js/apps/tambahan/form.js?_='.rand();
        $divisi     = DB::table('divisi')->where([['status', 1], ['urutan', '<>', 0]])->orderBy('urutan', 'asc')->get();

        return view('tambahan.form', compact('title', 'js', 'id', 'divisi'));
    }

    public function edit_tambahan(Request $request) 
    {
        $id     = $request->id;
        $user   = $this->order->getOrder($id);
    ...
    public function delete_tambahan(Request $request)
    {
        $id     = $request->id;
        $user   = Auth::user();
