...
use App\Repositories\FinishingSatuRepository;
use Carbon\Carbon;
use Yajra\DataTables\DataTables;

class FinishingSatuController extends BaseController
{
    protected $finishingrepo;
    protected $order;
    protected $order_detail;

    function __construct(FinishingSatuRepository $finishingrepo, Order $order, OrderDetail $order_detail)
    {
    ...
    {
        $title  = 'Finishing Satu';
        $js     = 'js/apps/finishing-satu/index.js?_='.rand();
        return view('finishing-satu.index', compact('js', 'title'));
    }

    public function datatable_finishing_satu(Request $request)
    {
        ...
        $data = $this->finishing->dataTableFinishingSatu($start_date, $end_date, $status, $order_by); 
        return Datatables::of($data)->addIndexColumn()
                ->addColumn('action', function($row) {

                    $btn_action = '';

                    if(Gate::allows('crudAccess', 'FS1', $row)) {
                        $user = Auth::user();
                        if(in_array($user->id_role, [1,2])) {
                            $btn_action .= '<li><a href="/finishing-satu/form/'.$row->uid.'" class="btn"><em class="icon ni ni-edit"></em><span>Edit</span></a></li>
    ...
    public function detail_finishing_satu(Request $request) 
    {
        $id     = $request->id;
    ...
    public function datatable_detail_finishing_satu(Request $request)
    {
        $uid    = $request->uid;
    ...
    public function form_finishing_satu(Request $request)
    {
        $title      = 'Form Job';
        $id         = $request->id;
        $js         = 'js/apps/finishing-satu/form.js?_='.rand();
        $divisi     = DB::table('divisi')->where([['status', 1], ['urutan', '<>', 0]])->orderBy('urutan', 'asc')->get();

        return view('finishing-satu.form', compact('title', 'js', 'id', 'divisi'));
    }

    public function edit_finishing_satu(Request $request) 
    {
        $id     = $request->id;
        $user   = $this->order->getOrder($id);
    ...
    public function delete_finishing_satu(Request $request)
    {
        $id     = $request->id;
        $user   = Auth::user();
    ...
    public function store_finishing_satu(Request $request)
    {
        $id = $request->input('id');
    ...
    public function approve_finishing_satu(Request $request)
    {
        $validator = Validator::make($request->all(), [
    ...
    public function pending_finishing_satu(Request $request)
    {
        $validator = Validator::make($request->all(), [
