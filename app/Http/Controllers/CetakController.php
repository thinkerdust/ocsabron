...
use App\Repositories\CetakRepository;
use Carbon\Carbon;
...
class CetakController extends BaseController
{
    ...
    function __construct(CetakRepository $cetakrepo, Order $order, OrderDetail $order_detail)
    {
        $this->cetak        = $cetakrepo;
        $this->order        = $order;
    ...
    {
        $title  = 'Cetak';
        $js     = 'js/apps/cetak/index.js?_='.rand();
        return view('cetak.index', compact('js', 'title'));
    }

    public function datatable_cetak(Request $request)
    {
        ...
        $data = $this->cetak->dataTableCetak($start_date, $end_date, $status, $order_by); 
                ...
                ->addColumn('action', function($row) {
                    ...
                    if(Gate::allows('crudAccess', 'CETAK', $row)) {
                        ...
                        if(in_array($user->id_role, [1,2])) {
                            $btn_action .= '<li><a href="/cetak/form/'.$row->uid.'" class="btn"><em class="icon ni ni-edit"></em><span>Edit</span></a></li>
    ...
    public function detail_cetak(Request $request) 
    {
        $id     = $request->id;
    ...
    public function datatable_detail_cetak(Request $request)
    {
        $uid    = $request->uid;
    ...
    public function form_cetak(Request $request)
    {
        $title      = 'Form Job';
        $id         = $request->id;
        $js         = 'js/apps/cetak/form.js?_='.rand();
        $divisi     = DB::table('divisi')->where([['status', 1], ['urutan', '<>', 0]])->orderBy('urutan', 'asc')->get();

        return view('cetak.form', compact('title', 'js', 'id', 'divisi'));
    }

    public function edit_cetak(Request $request) 
    {
        $id     = $request->id;
        $user   = $this->order->getOrder($id);
    ...
    public function delete_cetak(Request $request)
    {
        $id     = $request->id;
        $user   = Auth::user();
    ...
    public function approve_cetak(Request $request)
    {
        $validator = Validator::make($request->all(), [
    ...
    public function pending_cetak(Request $request)
    {
        $validator = Validator::make($request->all(), [
