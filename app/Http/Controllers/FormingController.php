...
use App\Repositories\FormingRepository;
use Carbon\Carbon;
use Yajra\DataTables\DataTables;

class FormingController extends BaseController
{
    protected $formingrepo;
    protected $order;
    protected $order_detail;

    function __construct(FormingRepository $formingrepo, Order $order, OrderDetail $order_detail)
    {
        $this->forming      = $formingrepo;
        $this->order        = $order;
    ...
    {
        $title  = 'Forming';
        $js     = 'js/apps/forming/index.js?_='.rand();
        return view('forming.index', compact('js', 'title'));
    }

    public function datatable_forming(Request $request)
    {
                        ...
                        $btn = '<div class="drodown">
                                ...
                                <div class="dropdown-menu dropdown-menu-end">
                                    <ul class="link-list-opt no-bdr">
                                        ...
                                        <li><a href="/forming/form/'.$row->uid.'" class="btn"><em class="icon ni ni-edit"></em><span>Edit</span></a></li>
    ...
    public function detail_forming(Request $request) 
    {
        $id     = $request->id;
    ...
    public function datatable_detail_forming(Request $request)
    {
        $uid    = $request->uid;
    ...
    public function form_forming(Request $request)
    {
        $title      = 'Form Job';
        $id         = $request->id;
        $js         = 'js/apps/forming/form.js?_='.rand();
        $divisi     = DB::table('divisi')->where([['status', 1], ['urutan', '<>', 0]])->orderBy('urutan', 'asc')->get();

        return view('forming.form', compact('title', 'js', 'id', 'divisi'));
    }

    public function edit_forming(Request $request) 
    {
        $id     = $request->id;
        $user   = $this->order->getOrder($id);
    ...
    public function delete_forming(Request $request)
    {
        $id     = $request->id;
        $user   = Auth::user();
    ...
    public function store_forming(Request $request)
    {
        $id = $request->input('id');
    ...
    public function pending_forming(Request $request)
    {
        $validator = Validator::make($request->all(), [
