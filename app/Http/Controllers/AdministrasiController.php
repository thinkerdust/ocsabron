...
class AdministrasiController extends BaseController
{
                ...
                ->addColumn('action', function($row) {
                    ...
                    if(Gate::allows('crudAccess', 'ADM', $row)) {
                        ...
                        if(in_array($user->id_role, [1,2])) {
                            $btn_action = '<li><a href="/administrasi/form/'.$row->uid.'" class="btn"><em class="icon ni ni-edit"></em><span>Edit</span></a></li>
    ...
    public function form_administrasi(Request $request)
    {
        $title      = 'Form Job';
        $id         = $request->id;
        $js         = 'js/apps/administrasi/form.js?_='.rand();
        $divisi     = DB::table('divisi')->where([['status', 1], ['urutan', '<>', 0]])->orderBy('urutan', 'asc')->get();

        return view('administrasi.form', compact('title', 'js', 'id', 'divisi'));
    }
    ...
    public function delete_administrasi(Request $request)
    {
        $id     = $request->id;
        $user   = Auth::user();
