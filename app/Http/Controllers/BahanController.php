...
class BahanController extends BaseController
{
    ...
    public function datatable_bahan(Request $request)
    {
        ...
        $data = $this->bahan->dataTableBahan($start_date, $end_date, $status, $order_by); 
                ...
                ->addColumn('action', function($row) {
                    ...
                    if(Gate::allows('crudAccess', 'BAHAN', $row)) {
                        ...
                        if(in_array($user->id_role, [1,2])) {
                            $btn_action .= '<li><a href="/bahan/form/'.$row->uid.'" class="btn"><em class="icon ni ni-edit"></em><span>Edit</span></a></li>
    ...
    public function form_bahan(Request $request)
    {
        $title      = 'Form Job';
        $id         = $request->id;
        $js         = 'js/apps/bahan/form.js?_='.rand();
        $divisi     = DB::table('divisi')->where([['status', 1], ['urutan', '<>', 0]])->orderBy('urutan', 'asc')->get();

        return view('bahan.form', compact('title', 'js', 'id', 'divisi'));
    }

    public function edit_bahan(Request $request) 
    {
        $id     = $request->id;
        $user   = $this->order->getOrder($id);
    ...
    public function delete_bahan(Request $request)
    {
        $id     = $request->id;
        $user   = Auth::user();
